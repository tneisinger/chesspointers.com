...
const ChessMoveSelector: React.FunctionComponent<Props> = ({
  ...
  if (moves.length > 1) {
      ...
      <FormControl component='fieldset'>
        ...
        >
          {moves.map((move) => (
            <FormControlLabel
              value={move.label}
              control={
                <Radio inputProps={{ 'data-testid': move.label } as MyInputProps} />
              }
              label={move.label}
              key={move.label}
        ...
        {selectedMove != null && (
          <button
            ...
            onClick={() =>
              handleSubmit(selectedMove.shortMove.from, selectedMove.shortMove.to)
            }
          >
            Submit Move Choice
          </button>
    ...
    return (
      <button
        onClick={() => handleSubmit(moves[0].shortMove.from, moves[0].shortMove.to)}
        data-testid={SELECT_BTN_TEST_ID}
      >
        Play Next Move
      </button>
